# Cali-Bud Delivery: Blog Deployment Order
...
- [ ] **Identity Check**: Verify `s3://cali-bud-deliveries` and `E48DSMEP69HP2` in [CLIENT_INTAKE.md](CLIENT_INTAKE.md) before pushing.
- [ ] **Dry Run**: `aws s3 sync . s3://cali-bud-deliveries --exclude ".git/*" --exclude ".github/*" --exclude "factory/*" --profile mediusa --dryrun`
- [ ] **Git Push**: `git add . && git commit -m "feat: deploy blog - [Topic]" && git push`
- [ ] **AWS Sync**: `aws s3 sync . s3://cali-bud-deliveries --delete --exclude ".git/*" --exclude ".github/*" --exclude "factory/*" --profile mediusa`
- [ ] **Invalidate**: `aws cloudfront create-invalidation --distribution-id E48DSMEP69HP2 --paths "/blog/*" "/news/*" --profile mediusa`
